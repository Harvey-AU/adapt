...
FLY_APPS=$(flyctl apps list | grep "adapt-app-pr-" | awk '{print $1}' || true)
...
for APP in $FLY_APPS; do
    # Extract PR number from app name (e.g., adapt-app-pr-114 -> 114)
    PR_NUM=$(echo "$APP" | sed 's/adapt-app-pr-//')
