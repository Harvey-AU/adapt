...
`~/.config/adapt-app-goodnative/auth/session.json` (or
`%APPDATA%\AdaptBot\auth\session.json` on Windows). All load-test scripts will
reuse it automatically. Re-run the command any time you need to refresh the
